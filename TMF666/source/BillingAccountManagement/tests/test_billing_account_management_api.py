...
from accountManagementEngineMicroservice.implementation.app import app as engine_app
from billingAccountManagementMicroservice.implementation.app import LISTENER_OPERATIONS, app
from common.accounts import resource_names
...
def sample_payloads() -> dict[str, dict]:
    return {
        "billFormat": {
            "name": "Print PDF",
            "description": "Printable PDF bill format",
            "formatType": "PDF",
            "@type": "BillFormat",
        },
        "billPresentationMedia": {
            "name": "SMS Delivery",
            "description": "Notify by SMS with portal follow-up",
            "mediumType": "sms",
            "billFormat": {"id": "bill-format-email-pdf"},
            "@type": "BillPresentationMedia",
        },
        "billingAccount": {
            "name": "Fabrikam Billing",
            "description": "Billing account for Fabrikam HQ",
            "accountBalance": {"amount": {"unit": "USD", "value": 99.5}},
            "relatedParty": [{"id": "org-2001", "role": "customer", "@referredType": "Organization"}],
            "@type": "BillingAccount",
        },
        "billingCycleSpecification": {
            "name": "Semiannual Billing",
            "description": "Semiannual billing cycle",
            "frequency": "semiannual",
            "billingDateShift": 10,
            "@type": "BillingCycleSpecification",
        },
        "financialAccount": {
            "name": "Receivables Ledger 2001",
            "description": "Receivables account for Fabrikam",
            "accountBalance": {"amount": {"unit": "USD", "value": 99.5}},
            "@type": "FinancialAccount",
        },
        "partyAccount": {
            "name": "Fabrikam Party Account",
            "description": "Commercial party account for Fabrikam",
            "relatedParty": [{"id": "org-2001", "role": "customer", "@referredType": "Organization"}],
            "@type": "PartyAccount",
        },
        "settlementAccount": {
            "name": "Settlement Account Central",
            "description": "Settlement account for central region",
            "defaultPaymentMethod": "wireTransfer",
            "@type": "SettlementAccount",
        },
    }


class BillingAccountManagementApiTest(unittest.TestCase):
    def setUp(self) -> None:
        get_store().reset()
    ...
    def test_api_directory_lists_crud_resources_and_listener_paths(self) -> None:
        ...
        self.assertIn("listBillingAccount", links)
        self.assertIn("createBillingAccount", links)
        self.assertIn("retrieveBillingAccount", links)
        self.assertIn("patchBillingAccount", links)
        self.assertIn("deleteBillingAccount", links)
    ...
    def test_invalid_payloads_are_rejected(self) -> None:
        invalid_create = asyncio.run(
            self.request(
                "POST",
                f"{API_BASE}/billPresentationMedia",
                json={"name": "broken", "billFormat": "not-an-object"},
            )
        )
        ...
        invalid_patch = asyncio.run(
            self.request(
                "PATCH",
                f"{API_BASE}/billingAccount/billing-account-1001",
                json={"id": "forbidden"},
            )
        )
    ...
    def test_hub_idempotency_and_listener_routes(self) -> None:
        payload = {"callback": "http://listener.local/events", "query": "eventType=BillingAccountCreateEvent"}
