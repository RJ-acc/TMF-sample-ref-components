...
from common.accounts import (
    ...
    apply_patch as apply_account_patch,
    build_entity,
    ensure_seed_data,
    resource_collection,
    resource_names,
...
logger = logging.getLogger("tmfc024-billing-account-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "billingaccountmanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/accountManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF666 - Account Management

### Reference component: TMFC024 Billing Account Management

Account Management exposes CRUD operations for billing accounts and related account-management resources:
bill formats, bill presentation media, billing accounts, billing cycle specifications, financial accounts,
party accounts, and settlement accounts.

### Operations
- List, create, retrieve, patch, and delete all TMF666 account resources
- Register and unregister event listeners through the hub resource
- Accept TMF666 listener callbacks for create, delete, state-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC024 Billing Account Management API",
...
@router.get("/", include_in_schema=False)
async def describe_account_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_account_patch(resource_name, existing, patch, API_BASE_PATH)
