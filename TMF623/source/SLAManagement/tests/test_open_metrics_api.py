...
class OpenMetricsApiTest(unittest.TestCase):
    def _load_app(self, base_path: str):
        ...
        try:
            ...
            return module.app, module.EVENT_COUNTER, module.STATE_COUNTER, module.SEVERITY_COUNTER
        finally:
            if previous is None:
    ...
    def test_metrics_available_with_and_without_base_path(self) -> None:
        app, event_counter, state_counter, severity_counter = self._load_app("/sm1-slamanagement")
        event_counter.clear()
        state_counter.clear()
        ...
        self.assertIn("tmf623_sla_events_total", direct_response.text)

        base_response = asyncio.run(self._request(app, "GET", "/sm1-slamanagement/metrics"))
        self.assertEqual(base_response.status_code, 200)
        self.assertIn("tmf623_sla_events_total", base_response.text)
