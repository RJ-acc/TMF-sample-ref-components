...
from common.sla import build_entity, resource_names
...
logger = logging.getLogger("tmf623-mcp")
...
CUSTOM_TOOLS = [
    types.Tool(
        name="list_all_tools",
        description="List all generated TMF623 MCP tools available on this server.",
        inputSchema={"type": "object", "properties": {}},
    ),
    types.Tool(
        name="validate_sla_payload",
        description="Validate a TMF623 create payload for one of the SLA-management resources.",
        inputSchema={
            "type": "object",
    ...
    types.Tool(
        name="validate_sla_patch_payload",
        description="Validate a TMF623 patch payload for one of the SLA-management resources.",
        inputSchema={
            "type": "object",
            "required": ["resource_name", "payload"],
            "properties": {
    ...
    types.Tool(
        name="validate_sla_replace_payload",
        description="Validate a TMF623 replace payload for one of the SLA-management resources.",
        inputSchema={
            "type": "object",
            "required": ["resource_name", "payload"],
            "properties": {
    ...
    types.Tool(
        name="preview_sla_payload",
        description="Preview the normalized TMF623 entity that would be created for an SLA-management resource.",
        inputSchema={
            "type": "object",
            "required": ["resource_name", "payload"],
            "properties": {
...
async def call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    if name == "validate_sla_payload":
        payload = validate_create_payload(args.get("resource_name", ""), args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "validate_sla_patch_payload":
        payload = validate_patch_payload(args.get("resource_name", ""), args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "validate_sla_replace_payload":
        payload = validate_replace_payload(args.get("resource_name", ""), args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "preview_sla_payload":
