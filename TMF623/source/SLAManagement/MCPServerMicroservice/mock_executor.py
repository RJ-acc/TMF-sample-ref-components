...
RESOURCE_DEFAULTS = {
    "sla": {"@type": "SLA", "name": "Mock SLA", "state": "active"},
    "slaviolation": {"@type": "SLAViolation", "name": "Mock SLA Violation", "state": "detected", "severity": "major"},
...
def _build_mock_item(resource: str, resource_id: str, arguments: dict[str, object], path: str) -> dict[str, object]:
    ...
    item = {
        "id": resource_id,
        "href": f"/tmf-api/slaManagement/v2{collection_path}/{resource_id}",
        "lastUpdate": _now(),
        **RESOURCE_DEFAULTS.get(resource, {"@type": resource}),
    }
    if resource in {"sla", "slaviolation"}:
...
def mock_execute(path: str, method: str, arguments: dict[str, object]) -> dict:
    ...
    note = "MOCK MODE - responses are synthetic. Set TMF623_MODE=live for real API calls."
    ...
    if method in {"post", "patch", "put"}:
        return {
            "success": True,
            "http_status": 200 if method in {"patch", "put"} else 201,
            "data": _build_mock_item(
                resource,
                str(resource_id) if method in {"patch", "put"} else _new_id(),
                arguments,
                path,
            ),
            "note": note,
