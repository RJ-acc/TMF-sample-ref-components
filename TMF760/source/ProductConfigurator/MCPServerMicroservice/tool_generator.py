...
LIST_ID_OVERRIDES = {
    "listQueryProductConfiguration": "list_query_product_configuration",
    "listCheckProductConfiguration": "list_check_product_configuration",
}

RESOURCE_HINTS = {
    "queryproductconfiguration": "Query Product Configuration provides computed configuration options for a given request context.",
    "checkproductconfiguration": "Check Product Configuration validates configured products and can return alternative proposals.",
    "hub": "The Hub resource manages event subscriptions for TMF760 notifications.",
}
    ...
    if method in {"post", "patch", "put"}:
        ...
        for raw_param in operation.get("parameters", []):
            ...
            if parameter.get("in") == "body":
                ...
                break
        if not body_schema and operation.get("requestBody"):
...
def generate_tools(spec: dict) -> dict[str, dict]:
    ...
    for path, path_item in spec.get("paths", {}).items():
        ...
        for method, operation in path_item.items():
            ...
            description_parts.append(f"TMF760 operation: {method.upper()} {path}")
