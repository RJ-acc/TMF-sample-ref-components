...
logger = logging.getLogger("tmf760-mcp")
...
CUSTOM_TOOLS = [
    types.Tool(
        name="list_all_tools",
        description="List all generated TMF760 MCP tools available on this server.",
        inputSchema={"type": "object", "properties": {}},
    ),
    types.Tool(
        name="validate_query_product_configuration_payload",
        description="Validate a TMF760 QueryProductConfiguration payload using the TMFC027 validation rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
    ...
    types.Tool(
        name="preview_query_product_configuration_payload",
        description="Generate a local TMFC027 query product configuration preview using the starter catalog rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
            "properties": {"payload": {"type": "object"}},
    ...
    types.Tool(
        name="validate_check_product_configuration_payload",
        description="Validate a TMF760 CheckProductConfiguration payload using the TMFC027 validation rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
            "properties": {"payload": {"type": "object"}},
    ...
    types.Tool(
        name="preview_check_product_configuration_payload",
        description="Run a local TMFC027 check product configuration preview using the starter catalog rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
            "properties": {"payload": {"type": "object"}},
...
async def call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    if name == "validate_query_product_configuration_payload":
        payload = validate_query_product_configuration(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "preview_query_product_configuration_payload":
        payload = generate_query_product_configuration(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "validate_check_product_configuration_payload":
        payload = validate_check_product_configuration(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "preview_check_product_configuration_payload":
        payload = evaluate_check_product_configuration(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]
