...
def _build_mock_item(resource: str, resource_id: str, arguments: dict[str, object], base_path: str) -> dict[str, object]:
    ...
    item = {
        "id": resource_id,
        "href": f"/tmf-api/productConfiguration/v5{collection_path}/{resource_id}",
        "lastUpdate": _now(),
...
def mock_execute(path: str, method: str, arguments: dict[str, object]) -> dict:
    ...
    note = "MOCK MODE - responses are synthetic. Set TMF760_MODE=live for real API calls."
    ...
    if method == "post":
        return {
            "success": True,
            "http_status": 200 if arguments.get("instantSync") else 201,
            "data": _build_mock_item(resource, _new_id(), arguments, path),
            "note": note,
        }
