...
from common.access import (
    ...
    apply_patch as apply_access_patch,
    build_entity,
    ensure_seed_data,
    resource_collection,
    resource_names,
...
logger = logging.getLogger("tmfc035-permissions-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "permissionsmanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/rolesAndPermissions/v4").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "4.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF672 - User Role Permission Management

### Reference component: TMFC035 Permissions Management

Permissions Management exposes the official TMF672 resources:
permission and userRole.

### Operations
- List, create, retrieve, and patch Permissions and UserRoles
- Register and unregister event listeners through the hub resource
- Accept TMF672 listener callbacks for create, delete, state-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC035 Permissions Management API",
...
@router.get("/", include_in_schema=False)
async def describe_permissions_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_access_patch(resource_name, existing, patch, API_BASE_PATH)
