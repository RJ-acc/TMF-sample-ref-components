...
from common.services import (
    ...
    apply_patch as apply_service_patch,
    build_entity,
    ensure_seed_data,
    resource_collection,
    resource_names,
...
logger = logging.getLogger("tmfc008-service-inventory-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "serviceinventorymanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/serviceInventoryManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF638 - Service Inventory

### Reference component: TMFC008 Service Inventory Management

Service Inventory Management exposes CRUD operations for the TMF638
service resource.

### Operations
- List, create, retrieve, patch, and delete Service records
- Register and unregister event listeners through the hub resource
- Accept TMF638 listener callbacks for create, delete, state-change, operating-status-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC008 Service Inventory Management API",
...
@router.get("/", include_in_schema=False)
async def describe_service_inventory_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_service_patch(resource_name, existing, patch, API_BASE_PATH)
        ...
        if updated.get("operatingStatus") != existing.get("operatingStatus"):
            await _publish_event(resource_name, "OperatingStatusChangeEvent", updated)
        return updated
