...
class OpenMetricsApiTest(unittest.TestCase):
    def _load_app(self, base_path: str):
        ...
        try:
            ...
            return module.app, module.EVENT_COUNTER, module.STATE_COUNTER
        finally:
            if previous is None:
    ...
    def test_metrics_available_with_and_without_base_path(self) -> None:
        app, event_counter, state_counter = self._load_app("/bg1-billgenerationmanagement")
        event_counter.clear()
        state_counter.clear()

        direct_response = asyncio.run(self._request(app, "GET", "/metrics"))
        self.assertEqual(direct_response.status_code, 200)
        self.assertIn("tmfc030_customer_bill_events_total", direct_response.text)

        base_response = asyncio.run(self._request(app, "GET", "/bg1-billgenerationmanagement/metrics"))
        self.assertEqual(base_response.status_code, 200)
        self.assertIn("tmfc030_customer_bill_events_total", base_response.text)
