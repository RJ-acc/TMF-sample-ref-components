...
LIST_ID_OVERRIDES = {
    "listAppliedCustomerBillingRate": "list_applied_customer_billing_rate",
    "listBillCycle": "list_bill_cycle",
    "listCustomerBill": "list_customer_bill",
    "listCustomerBillOnDemand": "list_customer_bill_on_demand",
}

RESOURCE_HINTS = {
    "appliedcustomerbillingrate": "Applied Customer Billing Rate exposes the rated charges that feed customer bill generation.",
    "billcycle": "Bill Cycle exposes the recurring billing cadence used when creating customer bills.",
    "customerbill": "Customer Bill exposes the rendered customer-facing bill and supports partial state updates.",
    "customerbillondemand": "Customer Bill On Demand triggers a bill generation task for a specific billing account.",
    "hub": "The Hub resource manages event subscriptions for TMF678 notifications.",
}
...
def generate_tools(spec: dict) -> dict[str, dict]:
    ...
    for path, path_item in spec.get("paths", {}).items():
        ...
        for method, operation in path_item.items():
            ...
            description_parts.append(f"TMF678 operation: {method.upper()} {path}")
