...
from common.billing import generate_customer_bill_on_demand
...
logger = logging.getLogger("tmf678-mcp")
...
CUSTOM_TOOLS = [
    types.Tool(
        name="list_all_tools",
        description="List all generated TMF678 MCP tools available on this server.",
        inputSchema={"type": "object", "properties": {}},
    ),
    types.Tool(
        name="validate_customer_bill_on_demand_payload",
        description="Validate a TMF678 CustomerBillOnDemand payload using the TMFC030 validation rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
    ...
    types.Tool(
        name="preview_customer_bill_on_demand_payload",
        description="Generate a local TMFC030 customer bill on demand preview using the starter billing rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
            "properties": {"payload": {"type": "object"}},
    ...
    types.Tool(
        name="validate_customer_bill_patch_payload",
        description="Validate a TMF678 CustomerBill patch payload using the TMFC030 validation rules.",
        inputSchema={
            "type": "object",
            "required": ["payload"],
            "properties": {"payload": {"type": "object"}},
...
async def call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    if name == "validate_customer_bill_on_demand_payload":
        payload = validate_customer_bill_on_demand(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "preview_customer_bill_on_demand_payload":
        payload = generate_customer_bill_on_demand(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]

    if name == "validate_customer_bill_patch_payload":
        payload = validate_customer_bill_patch(args.get("payload", {}))
        return [types.TextContent(type="text", text=json.dumps(payload, indent=2))]
