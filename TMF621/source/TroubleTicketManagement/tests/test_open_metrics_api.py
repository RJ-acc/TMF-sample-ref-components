...
class OpenMetricsApiTest(unittest.TestCase):
    ...
    def test_metrics_available_with_and_without_base_path(self) -> None:
        app, event_counter, status_counter = self._load_app("/tt1-troubleticketmanagement")
        event_counter.clear()
        ...
        self.assertIn("tmf621_trouble_ticket_events_total", direct_response.text)

        base_response = asyncio.run(self._request(app, "GET", "/tt1-troubleticketmanagement/metrics"))
        self.assertEqual(base_response.status_code, 200)
        self.assertIn("tmf621_trouble_ticket_events_total", base_response.text)
