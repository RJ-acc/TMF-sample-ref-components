...
RESOURCE_DEFAULTS = {
    "troubleticket": {
        "@type": "TroubleTicket",
        "name": "Mock trouble ticket",
        "description": "Synthetic mock trouble ticket response.",
        "severity": "major",
        "status": "acknowledged",
    },
    "troubleticketspecification": {
        "@type": "TroubleTicketSpecification",
        "name": "Mock trouble ticket specification",
        "description": "Synthetic mock trouble ticket specification response.",
        "version": "1.0",
...
def _build_mock_item(resource: str, resource_id: str, arguments: dict[str, object], path: str) -> dict[str, object]:
    ...
    item = {
        "id": resource_id,
        "href": f"/tmf-api/troubleTicket/v5{collection_path}/{resource_id}",
        "lastUpdate": _now(),
        **RESOURCE_DEFAULTS.get(resource, {"@type": resource}),
    }
    if resource == "troubleticket":
        item["creationDate"] = _now()
        item["statusChange"] = {
            "@type": "StatusChange",
            "status": item.get("status", "acknowledged"),
            "changeDate": _now(),
            "changeReason": "Mock response generated",
        }
...
def mock_execute(path: str, method: str, arguments: dict[str, object]) -> dict:
    ...
    note = "MOCK MODE - responses are synthetic. Set TMF621_MODE=live for real API calls."
    ...
    if method in {"post", "patch"}:
        return {
            ...
            "data": _build_mock_item(resource, str(resource_id) if method == "patch" else _new_id(), arguments, path),
            "note": note,
        }
