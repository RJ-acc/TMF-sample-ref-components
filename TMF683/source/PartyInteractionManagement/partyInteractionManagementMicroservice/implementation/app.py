...
logger = logging.getLogger("tmfc023-party-interaction-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "partyinteractionmanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/partyInteractionManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF683 - Party Interaction

### Reference component: TMFC023 Party Interaction Management

Party Interaction Management exposes CRUD operations for the TMF683
partyInteraction resource.

### Operations
- List, create, retrieve, patch, and delete PartyInteraction records
- Register and unregister event listeners through the hub resource
- Accept TMF683 listener callbacks for create, delete, status-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC023 Party Interaction Management API",
...
@router.get("/", include_in_schema=False)
async def describe_party_interaction_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_interaction_patch(resource_name, existing, patch, API_BASE_PATH)
