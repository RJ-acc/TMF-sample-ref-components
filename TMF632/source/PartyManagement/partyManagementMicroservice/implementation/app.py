...
from common.parties import (
    LISTENER_EVENT_SUFFIXES,
    apply_patch as apply_party_patch,
    build_entity,
    ensure_seed_data,
    resource_collection,
    resource_names,
...
logger = logging.getLogger("tmfc028-party-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "partymanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/partyManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF632 - Party Management

### Reference component: TMFC028 Party Management

Party Management exposes CRUD operations for the two mandatory TMF632 resources:
individual and organization.

### Operations
- List, create, retrieve, patch, and delete Individuals and Organizations
- Register and unregister event listeners through the hub resource
- Accept TMF632 listener callbacks for create, delete, state-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC028 Party Management API",
...
@router.get("/", include_in_schema=False)
async def describe_party_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_party_patch(resource_name, existing, patch, API_BASE_PATH)
