...
app = FastAPI(title="TMFC028 Metrics Listener", docs_url=None, redoc_url=None)
...
def _render_metrics() -> str:
    lines = [
        "# HELP tmfc028_party_events_total TMFC028 party-management events observed by the metrics listener",
        "# TYPE tmfc028_party_events_total counter",
    ]

    if EVENT_COUNTER:
        for key, count in sorted(EVENT_COUNTER.items()):
            resource_type, event_type = key.split(":", 1)
            lines.append(f'tmfc028_party_events_total{{resource="{resource_type}",eventType="{event_type}"}} {count}')
    else:
        lines.append('tmfc028_party_events_total{resource="none",eventType="none"} 0')

    lines.extend(
        [
            "# HELP tmfc028_party_states_total TMFC028 party states observed from incoming events",
            "# TYPE tmfc028_party_states_total counter",
        ]
    )
    ...
    if STATE_COUNTER:
        for key, count in sorted(STATE_COUNTER.items()):
            ...
            lines.append(f'tmfc028_party_states_total{{resource="{resource_type}",state="{state}"}} {count}')
    else:
        lines.append('tmfc028_party_states_total{resource="none",state="none"} 0')
