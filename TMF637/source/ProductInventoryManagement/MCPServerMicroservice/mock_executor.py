...
RESOURCE_DEFAULTS = {
    "product": {
        "@type": "Product",
        "name": "Mock Fiber Product",
        "description": "Synthetic TMF637 product instance returned by the MCP mock executor.",
        "status": "active",
        "productOffering": {"id": "product-offering-mock", "name": "Mock Offer", "@referredType": "ProductOffering"},
        "relatedParty": [{"role": "customer", "id": "party-mock", "name": "Mock Customer"}],
    },
...
def _build_mock_item(resource: str, resource_id: str, arguments: dict[str, object], path: str) -> dict[str, object]:
    ...
    item = {
        "id": resource_id,
        "href": f"/tmf-api/productInventoryManagement/v5{collection_path}/{resource_id}",
        "creationDate": _now(),
        "lastUpdate": _now(),
        "statusChangeDate": _now(),
...
def mock_execute(path: str, method: str, arguments: dict[str, object]) -> dict:
    ...
    note = "MOCK MODE - responses are synthetic. Set TMF637_MODE=live for real API calls."
