...
from common.products import (
    LISTENER_EVENT_SUFFIXES,
    apply_patch as apply_product_patch,
    build_entity,
    ensure_seed_data,
    resource_collection,
    resource_names,
...
logger = logging.getLogger("tmfc005-product-inventory-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "productinventorymanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/productInventoryManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF637 - Product Inventory

### Reference component: TMFC005 Product Inventory Management

Product Inventory Management exposes CRUD operations for the TMF637
product resource.

### Operations
- List, create, retrieve, patch, and delete Product records
- Register and unregister event listeners through the hub resource
- Accept TMF637 listener callbacks for create, delete, state-change, attribute-value-change, and product-batch events
"""

app = FastAPI(
    title="TMFC005 Product Inventory Management API",
...
@router.get("/", include_in_schema=False)
async def describe_product_inventory_api() -> dict[str, Any]:
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_product_patch(resource_name, existing, patch, API_BASE_PATH)
