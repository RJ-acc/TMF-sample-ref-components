...
from common.products import resource_names
from common.store import get_store
from productInventoryManagementEngineMicroservice.implementation.app import app as engine_app
from productInventoryManagementMicroservice.implementation.app import LISTENER_OPERATIONS, app
...
def sample_payload() -> dict[str, object]:
    return {
        "@type": "Product",
        "name": "Fiber 250 Access",
        "description": "Active residential fiber access product with 250 Mbps profile.",
        "status": "active",
        "isBundle": False,
        "isCustomerVisible": True,
        "startDate": "2026-04-09T10:00:00Z",
        "productSerialNumber": "ONT-250-1003",
        "productOffering": {
            "@type": "ProductOfferingRef",
            "id": "product-offering-fiber-250",
            "name": "Fiber 250",
            "@referredType": "ProductOffering",
        },
        "productSpecification": {
            "@type": "ProductSpecificationRef",
            "id": "product-specification-fiber-250",
            "name": "Fiber 250 Spec",
...
class ProductInventoryManagementApiTest(unittest.TestCase):
    def setUp(self) -> None:
        get_store().reset()
    ...
    def test_api_directory_lists_crud_resources_and_listener_paths(self) -> None:
        ...
        self.assertIn("listProduct", links)
        self.assertIn("createProduct", links)
        self.assertIn("retrieveProduct", links)
        self.assertIn("patchProduct", links)
        self.assertIn("deleteProduct", links)
        for operation_name, _ in LISTENER_OPERATIONS:
            self.assertIn(operation_name, links)

    def test_product_supports_full_crud(self) -> None:
        list_response = asyncio.run(self.request("GET", f"{API_BASE}/product"))
        self.assertEqual(list_response.status_code, 200)
        self.assertGreaterEqual(len(list_response.json()), 2)

        create_response = asyncio.run(self.request("POST", f"{API_BASE}/product", json=sample_payload()))
        self.assertEqual(create_response.status_code, 201, create_response.text)
        entity = create_response.json()
        self.assertEqual(entity["@type"], "Product")
        self.assertTrue(entity["href"].endswith(f"/product/{entity['id']}"))

        retrieve_response = asyncio.run(
            self.request("GET", f"{API_BASE}/product/{entity['id']}", params={"fields": "id,name,status"})
        )
        ...
        patch_response = asyncio.run(
            self.request(
                "PATCH",
                f"{API_BASE}/product/{entity['id']}",
                json={"description": "patched product", "status": "suspended"},
            )
        )
        self.assertEqual(patch_response.status_code, 200, patch_response.text)
        self.assertEqual(patch_response.json()["status"], "suspended")

        delete_response = asyncio.run(self.request("DELETE", f"{API_BASE}/product/{entity['id']}"))
        self.assertEqual(delete_response.status_code, 204)

        missing_response = asyncio.run(self.request("GET", f"{API_BASE}/product/{entity['id']}"))
    ...
    def test_list_filters_fields_and_pagination(self) -> None:
        response = asyncio.run(
            self.request(
                "GET",
                f"{API_BASE}/product",
                params={"fields": "id,name,status", "offset": 0, "limit": 1},
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 1)
        self.assertEqual(sorted(response.json()[0].keys()), ["id", "name", "status"])
    ...
    def test_invalid_payloads_are_rejected(self) -> None:
        invalid_create = asyncio.run(
            self.request(
                "POST",
                f"{API_BASE}/product",
                json={"@type": "Product", "name": "broken", "productOffering": "nope", "relatedParty": []},
            )
        )
        ...
        invalid_patch = asyncio.run(
            self.request(
                "PATCH",
                f"{API_BASE}/product/product-fiber-1001",
                json={"id": "forbidden"},
            )
        )
    ...
    def test_hub_idempotency_and_listener_routes(self) -> None:
        payload = {"callback": "http://listener.local/events", "query": "eventType=ProductCreateEvent"}
            ...
            with self.subTest(resource=resource_name):
                ...
                validate_patch = asyncio.run(
                    self.engine_request(
                        ...
                        json={"description": "patched product", "status": "active"},
                    )
                )
