...
COMPONENT_NAME = os.environ.get(
    ...
    else f"{RELEASE_NAME}-productcatalogmanagement",
)
API_VERSION = os.environ.get("TMF620_API_VERSION", "v4")
...
# Constants
configured_base_url = (
    os.environ.get("TMF620_BASE_URL")
    or os.environ.get("PRODUCT_CATALOG_BASE_URL")
    or os.environ.get("CATALOG_API_BASE_URL")
)

if configured_base_url:
    API_URL = configured_base_url.rstrip("/")
    if "/tmf-api/" not in API_URL:
        API_URL = f"{API_URL}/tmf-api/productCatalogManagement/{API_VERSION}"
elif RELEASE_NAME == "local":
    API_URL = f"https://localhost/{COMPONENT_NAME}/tmf-api/productCatalogManagement/{API_VERSION}"
else:
    api_service = os.environ.get(
        "PRODUCT_CATALOG_SERVICE",
        f"{RELEASE_NAME}-productcatalogapi",
    )
    API_URL = f"http://{api_service}:8080/{COMPONENT_NAME}/tmf-api/productCatalogManagement/{API_VERSION}"
