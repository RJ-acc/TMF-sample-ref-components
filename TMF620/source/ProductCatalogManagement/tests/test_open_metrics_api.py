...
class OpenMetricsApiTest(unittest.TestCase):
    ...
    def test_metrics_available_with_and_without_base_path(self) -> None:
        app, event_counter, state_counter = self._load_app("/pc1-productcatalogmanagement")
        event_counter.clear()
        state_counter.clear()

        direct_response = asyncio.run(self._request(app, "GET", "/metrics"))
        self.assertEqual(direct_response.status_code, 200)
        self.assertIn("tmfc001_catalog_events_total", direct_response.text)

        base_response = asyncio.run(self._request(app, "GET", "/pc1-productcatalogmanagement/metrics"))
        self.assertEqual(base_response.status_code, 200)
        self.assertIn("tmfc001_catalog_events_total", base_response.text)
