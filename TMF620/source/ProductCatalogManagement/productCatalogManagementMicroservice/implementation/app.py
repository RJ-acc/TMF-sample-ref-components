...
from common.catalog import apply_patch as apply_catalog_patch
...
logger = logging.getLogger("tmfc001-product-catalog-management-api")

COMPONENT_NAME = os.getenv("COMPONENT_NAME", "productcatalogmanagement")
API_BASE_PATH = "/" + os.getenv("API_BASE_PATH", "/tmf-api/productCatalogManagement/v5").strip("/")
ENGINE_SERVICE_URL = os.getenv("ENGINE_SERVICE_URL", "").rstrip("/")
API_VERSION = "5.0.0"
API_DIRECTORY_DESCRIPTION = """## TMF API Reference: TMF620 - Product Catalog Management

### Reference component: TMFC001 Product Catalog Management

Product Catalog Management exposes CRUD operations for product catalogs, categories, product offerings,
product offering prices, and product specifications, along with task-style import and export jobs.

### Operations
- List, create, retrieve, patch, and delete the main TMF620 catalog resources
- List, create, retrieve, and delete TMF620 import and export job task resources
- Register and unregister event listeners through the hub resource
- Accept TMF620 listener callbacks for create, delete, state-change, and attribute-value-change events
"""

app = FastAPI(
    title="TMFC001 Product Catalog Management API",
...
def _listener_operations() -> list[tuple[str, str]]:
    ...
    for resource_name in resource_names():
        ...
        for suffix in listener_suffixes(resource_name):
            operations.append(
                (
                    f"listenTo{entity_type}{suffix}",
                    f"/listener/{resource_name}{suffix}",
                )
            )
    return operations
...
async def _publish_event(resource_name: str, suffix: str, payload: dict[str, Any]) -> None:
    if not supports_event(resource_name, suffix):
        return
...
@router.get("/", include_in_schema=False)
async def describe_catalog_api() -> dict[str, Any]:
    ...
    for resource_name in resource_names():
        ...
        operations.extend(
            [
                ...
                },
                {
                    ...
                    "href": f"/{resource_name}",
                    "method": "POST",
                    "description": f"Creates a {entity_type}",
                },
                {
        ...
        if supports_patch(resource_name):
            operations.append(
                {
                    "name": f"patch{entity_type}",
                    "href": f"/{resource_name}/{{id}}",
                    "method": "PATCH",
                    "description": f"Updates partially a {entity_type}",
                }
            )
        operations.append(
            {
                ...
                "href": f"/{resource_name}/{{id}}",
                "method": "DELETE",
                "description": f"Deletes a {entity_type}",
            }
        )
...
def _create_handler(resource_name: str):
    ...
    async def handler(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if entity.get("state") not in {None, ""}:
            await _publish_event(resource_name, "StateChangeEvent", entity)
        return entity
...
def _patch_handler(resource_name: str):
    ...
    async def handler(entity_id: str, patch: dict[str, Any]) -> dict[str, Any]:
        ...
        updated = apply_catalog_patch(resource_name, existing, patch, API_BASE_PATH)
