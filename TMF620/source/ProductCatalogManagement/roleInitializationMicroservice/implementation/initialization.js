...
const canvasSystemRole = process.env.CANVAS_SYSTEM_ROLE || "canvasRole";
const usePermissionSpec = process.env.USE_PERMISSION_SPEC === 'true'; // Environment variable to determine which API to use
initialPartyRole.name = canvasSystemRole;
initialPermissionSpecificationSet.name = canvasSystemRole;
initialPermissionSpecificationSet.involvementRole = canvasSystemRole;
initialPermissionSpecificationSet.description = `${canvasSystemRole} permission specification set with read-only access rights`;
initialPermissionSpecificationSet.permissionSpecification[0].name = `${canvasSystemRole}:read-only`;
initialPermissionSpecificationSet.permissionSpecification[0].function = canvasSystemRole;
  ...
  while (complete == false) {
    try {
        ...
        if (usePermissionSpec) {
          ...
          const baseUrl = process.env.PERMISSION_SPEC_API_BASE_URL ||
            `http://${releaseName}-${apiName}:8080/${componentName}/rolesAndPermissionsManagement/v5`;
          url = `${baseUrl.replace(/\/$/, '')}/permissionSpecificationSet`;
          ...
          const baseUrl = process.env.ROLE_API_BASE_URL ||
            `http://${releaseName}-${apiName}:8080/${componentName}/tmf-api/partyRoleManagement/v4`;
          url = `${baseUrl.replace(/\/$/, '')}/partyRole`;
